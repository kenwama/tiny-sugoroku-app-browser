...
function setPlayingState() {
  ...
  const getStage = (() => {
    if (checkDefalutStage()) return defalutStage;
...
function checkDefalutStage() {
  ...
  const checkStage = sessionStorage.getItem("play-stage");

  if (checkSelecet || checkStage === "defalut") {
    return defalutStage;
  }
...
function displayStartModal() {
  ...
  title.classList.add("start-modal-title");
  title.textContent = "遊び方";

  const lists = document.createElement("ul");

  const contents = [
    "ユーザーの1人がゴールした時点でゲーム終了です",
    "ぴったりゴールなどのギミックはありません",
    "イベントは連鎖しません(2マス進む > 1マス戻るなどにはならない)",
    "場所交換イベントは互いのマスにユーザーいる場合のみ発生します",
    "基本的に全てのコマは手動で、毎回サイコロのボタンを押して進めてください",
    "コマが進むアニメーションの時に、スクロールの挙動がおかしい場合があるのでその時はリロードしてください",
  ];

  contents.map((content) => {
    const list = document.createElement("li");
    list.textContent = content;
    list.classList.add("start-modal-list");
    lists.append(list);
  });

  const button = document.createElement("button");
  button.classList.add("start-modal-button");
...
function PanelEventLavel(index) {
  ...
  const getHasEventState = allPanelEvent.filter(
    (panel) => panel.panelCount === index && panel
  );
...
function swapUser(target, user) {
  const exitAllTargetUser = moveabelAllUser.filter(
    (user) => user.currentPanel === target && user
  );
...
function checkPanelEvent(panelCount) {
  ...
  const getPanel = allPanelEvnet.filter(
    (panel) => panel.panelCount === panelCount
  );
