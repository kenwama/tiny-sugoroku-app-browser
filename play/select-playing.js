...
function DefalutStageButton() {
  ...
  defalutStageButton.addEventListener("click", () =>
    useSessionStorage("stage", "defalut")
  );
  defalutStageButton.addEventListener("click", () =>
    changeButtonUseHash("stage")
  );
...
function StageButton(stage) {
  ...
  button.addEventListener("click", (e) =>
    useSessionStorage("stage", e.target.id)
  );
...
function NumberOfUser() {
  ...
  modeNumberOfUser.map((number) => {
    ...
    button.addEventListener("click", (e) =>
      useSessionStorage("number-of-user", e.target.id)
    );
    button.addEventListener("click", () =>
      changeButtonUseHash("number-of-user")
    );
...
function UserIcon() {
  ...
  userIcons.map((icon) => {
    ...
    button.addEventListener("click", (e) =>
      useSessionStorage("user-icon", e.target.id)
    );
...
function useSessionStorage(selectString, selectId) {
  ...
  if (selectString === "number-of-user")
    sessionStorage.setItem("play-number-of-user", selectId);
  if (selectString === "user-icon")
    sessionStorage.setItem("play-user-icon", selectId);
}
