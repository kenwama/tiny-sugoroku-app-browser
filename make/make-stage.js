...
function setStageState() {
  ...
  const allPanelState = allPanel
    .filter((panel) => panel.state)
    .map((panel) => panel.state);

  const stageNamePrompt = prompt(
    "ステージの名前を入力してください\n20文字以内にしてください。"
  );
...
function init() {
  ...
  if (getStage !== "new-stage") {
    ...
    const filtered = getAllStage.filter(
      (stage) => stage.id === getStage && stage
    );
...
function NewStageMap() {
  // 選択したパネルの数をもとにマップを作る
  const getNumberOfPanel = document.querySelector(
...
function checkCanEditPanel(calcType) {
  let targetPanelId;
  if (calcType === "incremental")
    targetPanelId = currentPanelCount + targetPanelCount + 1;
  if (calcType === "decremental")
    targetPanelId = currentPanelCount + targetPanelCount - 1;

  const targetPanel = document.getElementById(targetPanelId);
  const editingCurrentPanel =
    targetPanel && targetPanel.classList.contains("editing");
  ...
  if (targetPanel && !editingCurrentPanel && !notEditablePanel)
    return targetPanel;
}
...
function decidePanelState() {
  ...
  if (currentEventType === "swap")
    panelState.targetCount = currentPanelCount + targetPanelCount;
  ...
  if (currentEventType === "swap") {
    const targetPanel = document.getElementById(
      targetPanelCount + currentPanelCount
    );
    const panelState = {};
...
function resetPanelState(panelId) {
  ...
  eventButton.addEventListener(
    "click",
    (e) => (currentPanelCount = e.target.buttonId)
  );
  eventButton.addEventListener("click", SelectEventModal);
